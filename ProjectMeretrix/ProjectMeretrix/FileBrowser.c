...
void FileBrowser_Update(FileBrowser* fb)
{
	...
	if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
	{
		...
		if (CheckCollisionPointRec(mouse, fb->panelRectangle))
		{
			...
			int row = (int)(mouse.y - fb->panelRectangle.y) / (ICON_SIZE + ICON_PADDING + LABEL_HEIGHT);
...
void FileBrowser_Draw(FileBrowser* fb)
{
	...
	for (int i = 0; i < fb->fileCount; i++) {
		...
		float y = fb->panelRectangle.y + 32 + row * (ICON_SIZE + ICON_PADDING + LABEL_HEIGHT);
		...
		DrawText(fb->files[i].name, x, y + ICON_SIZE + 2, 10, WHITE);
	}
